...
test("provides a persistent hierarchical travel footprint map", async () => {
  ...
  assert.doesNotMatch(map, /clearHoverOutsideMap/);
  assert.doesNotMatch(map, /map-admin-outer-outline/);
  assert.doesNotMatch(map, /adminOuterBoundary/);
  ...
  assert.doesNotMatch(styles, /\.map-hover-outline/);
  assert.doesNotMatch(styles, /\.map-selected-outline/);
  assert.match(styles, /\.map-admin:not\(\.is-visited\)/);
  assert.match(styles, /\.map-country-jump select/);
  assert.doesNotMatch(styles, /\.map-visited-outline/);
